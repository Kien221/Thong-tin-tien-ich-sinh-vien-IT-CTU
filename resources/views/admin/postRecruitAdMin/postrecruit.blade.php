       ...
       <div class="row contain">
        ...
        <div class="col-xs-9">
            <div style="display:flex;justify-content: space-between">
                <button>
                    <a href="{{route('admin.recuit.add')}}">Thêm Bài Tuyển Dụng</a>
                </button>
                <div class="row">
...
<script>
    $(document).ready(function() {
        let postrecuit_table = $('#customers').DataTable({
            processing: true,
            serverSide: true,
            select:true, 
            ajax: '{!! route('admin.postrecuit.api') !!}',
            columns: [
                {data:'stt', name: 'stt' },
    ...
    postrecuit_table.on( 'draw.dt', function () {
    var PageInfo = $('#customers').DataTable().page.info();
    let i = 1;
        postrecuit_table.column(0, { page: 'current' }).nodes().each( function (cell, i) {
